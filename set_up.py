...
def install_dvc():
    # Install DVC and its dependencies through poetry
    subprocess.run(['poetry', 'add', 'dvc[all]'], check=True)  # Changed this line
    # Initialize DVC
    subprocess.run(['dvc', 'init'], check=True)
    
    # Create basic .dvc config in data directory
    with open('data/.gitignore', 'w') as f:
        f.write('*\n')
    with open('data/.dvcignore', 'w') as f:
        f.write('# Add patterns of files dvc should ignore\n')
